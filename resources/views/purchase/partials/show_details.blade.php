<div class="modal-header">
    ...
    @php
      ...
      $currency_factor = !empty($purchase->exchange_rate) ? (float) $purchase->exchange_rate : 1;
    @endphp

    <h4 class="modal-title" id="modalTitle">
      {{$title}} (<b>@lang('purchase.ref_no'):</b> #{{ e($purchase->ref_no) }})
    </h4>
</div>
...
<div class="modal-body">
  <div class="row">
    <div class="col-sm-12">
      <p class="pull-right">
        <b>@lang('messages.date'):</b> {{ @format_date($purchase->transaction_date) }}
      </p>
    </div>
  </div>
  ...
  <div class="row invoice-info">
    <div class="col-sm-4 invoice-col">
      ...
      @if($purchase->document_path)
        <a href="{{$purchase->document_path}}"
           ...
           class="tw-dw-btn tw-dw-btn-success tw-text-white tw-dw-btn-sm pull-left no-print">
    ...
    <div class="col-sm-4 invoice-col">
      ...
      @if(!empty($purchase->exchange_rate))
        <b>Tipo de cambio:</b> {{ number_format($purchase->exchange_rate, 3) }}<br/>
      @endif
      ...
      @if($purchase->type == 'purchase_order')
        ...
        <span class="label @if(!empty($shipping_status_colors[$purchase->shipping_status])) {{$shipping_status_colors[$purchase->shipping_status]}} @else {{'bg-gray'}} @endif">
          {{$shipping_statuses[$purchase->shipping_status] ?? '' }}
        </span><br>
      ...
      <div class="table-responsive">
        <table class="table bg-gray">
          ...
          @foreach($purchase->purchase_lines as $purchase_line)
            @php
              $line_pp_without_discount = $purchase_line->pp_without_discount * $currency_factor;
              $line_purchase_price = $purchase_line->purchase_price * $currency_factor;
              $line_subtotal_before_tax = ($purchase_line->quantity * $purchase_line->purchase_price) * $currency_factor;
                ...
                @if($purchase_line->product->unit->sub_units)
                  @foreach($purchase_line->product->unit->sub_units as $sub_unit)
                    @if($sub_unit->id == $purchase_line->sub_unit_id)
                      ({{ (float) $sub_unit->base_unit_multiplier }} {{ $purchase_line->product->unit->short_name }})
                    @endif
                  @endforeach
                @endif
        ...
        <table class="table">
          ...
          @if(!empty($purchase->additional_expense_value_2) && !empty($purchase->additional_expense_key_2))
            <tr>
              ...
              <td><b>(+)</b></td>
              <td>
                <span class="display_currency pull-right">{{ $purchase->additional_expense_value_2 * $currency_factor }}</span>
              </td>
            </tr>
          @endif
          ...
          @if(!empty($purchase->additional_expense_value_3) && !empty($purchase->additional_expense_key_3))
            <tr>
              ...
              <td><b>(+)</b></td>
              <td>
                <span class="display_currency pull-right">{{ $purchase->additional_expense_value_3 * $currency_factor }}</span>
              </td>
            </tr>
          @endif
          ...
          @if(!empty($purchase->additional_expense_value_4) && !empty($purchase->additional_expense_key_4))
            <tr>
              ...
              <td><b>(+)</b></td>
              <td>
                <span class="display_currency pull-right">{{ $purchase->additional_expense_value_4 * $currency_factor }}</span>
              </td>
            </tr>
          @endif
          ...
          <tr>
            ...
            <td></td>
            <td>
              <span class="display_currency pull-right" data-currency_symbol="true">{{ $purchase->final_total * $currency_factor }}</span>
            </td>
          </tr>
        </table>
      </div>
