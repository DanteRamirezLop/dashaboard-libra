...
<input type="hidden" id="item_addition_method" value="{{$business_details->item_addition_method}}">
					...
					<table class="table table-condensed table-bordered table-striped table-responsive" id="pos_table">
						...
						<tbody>
							@foreach($sell_details as $sell_line)
								@include('sale_pos.product_row', ['product' => $sell_line, 'row_count' => $loop->index, 'tax_dropdown' => $taxes, 'sub_units' => !empty($sell_line->unit_details) ? $sell_line->unit_details : [], 'action' => 'edit', 'is_direct_sell' => true, 'so_line' => $sell_line->so_line, 'is_sales_order' => $transaction->type == 'sales_order', 'is_serial_no' => true, 'is_cyber_combo' => $sell_line->is_cyber_combo])
							@endforeach
						</tbody>
					</table>
					</div>
	...
	@can('sell.payments')
		...
		<div class="row">
			@php
				$actual_payments = array_filter($payment_lines, fn($p) => $p['is_return'] != 1);
				$payment_count = count($actual_payments);
			@endphp
			...
			@foreach($payment_lines as $payment_line)
				...
				<div class="payment_row" id="payment_rows_div">
					...
					@if($payment_count > 1)
						@include('sale_pos.partials.multiple_payment_row_form', ['row_index' => $loop->index, 'show_date' => true, 'payment_line' => $payment_line])
					@else
